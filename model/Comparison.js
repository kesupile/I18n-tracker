const { addOmission } = require("./PrototypeMethods");
require("colors");
...
Comparison.prototype.isNotOmitted = function(nest) {
  ...
  return !this.nestOmissions.find(
    nestOmission => nest.id === nestOmission.id && !nestOmission.doesExist
  );
};
...
Comparison.prototype.logOmissions = function() {
  ...
  console.log(`  ${String(this.root).red} \n${String(omString).yellow}`);
};
